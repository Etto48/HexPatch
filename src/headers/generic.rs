...
impl GenericHeader
{
    ...
    pub fn parse_header(bytes: &[u8], file_path: &str, filesystem: &FileSystem) -> Option<Self>
    {
        ...
        if let Ok(header) = header
        {
            ...
            let bitness = if header.is_64() { Bitness::Bit64 } else { Bitness::Bit32 };

            let endianness = header.endianness();

            let entry = header.entry();

            let sections = header.sections().map(|section|
            {
                Section
                {
            ...
            let mut symbols: Vec<(u64, String)> = header.symbols().map(|symbol|
            {
                    ...
                    if let Some(pdb_file_path) = pdb_file_path
                    {
                        ...
                        if let Ok(file) = file
                        {
                            ...
                            if let Ok(mut pdb) = pdb
                            {
                                ...
                                if let (Ok(symbol_table), Ok(address_map)) = (symbol_table, address_map)
                                {
                                    let mut iter = symbol_table.iter();
                                    while let Ok(Some(symbol)) = iter.next()
                                    {
                                        if let Ok(pdb::SymbolData::Public(public_symbol)) = symbol.parse()
                                        {
            ...
            let symbols: HashMap<u64, String> = symbols.into_iter().map(
                |(address, name)| (address, Self::demangle_symbol(&name))
            ).collect();

            let symbols_by_name = symbols.iter().map(
                |(address, name)| (name.clone(), *address)
            ).collect();
